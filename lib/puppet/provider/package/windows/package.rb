    ...
    def self.with_key(&block)
      %w[HKEY_LOCAL_MACHINE HKEY_CURRENT_USER].each do |hive|
        [KEY64, KEY32].each do |mode|
          mode |= KEY_READ
          begin
            Puppet::Util::Windows::Registry.open(hive, 'Software\Microsoft\Windows\CurrentVersion\Uninstall', mode) do |uninstall|
              each_key(uninstall) do |name, wtime|
                Puppet::Util::Windows::Registry.open(hive, "#{uninstall.keyname}\\#{name}", mode) do |key|
