...
puppet resource [-h|--help] [-d|--debug] [-v|--verbose] [-e|--edit]
  [-p|--param <parameter>] [-t|--types] [-y|--to_yaml] [-j|--to_json] <type>
...
* --to_json:
  Output found resources in JSON format.
...
Copyright (c) 2011 Puppet Inc., LLC Licensed under the Apache 2.0 License
  ...
  def main
    ...
    Puppet.override(:current_environment => env, :loaders => Puppet::Pops::Loaders.new(env)) do
      ...
      raise _("Editing with JSON output is not supported") if options[:edit] && options[:to_json]

      resources = find_or_save_resources(type, name, params)

      if options[:to_yaml]
        data = to_hiera_hash(resources)
        text = YAML.dump(type.downcase => data)
      elsif options[:to_json]
        data = to_hiera_hash(resources)
        text = Puppet::Util::Json.dump({type.downcase => data}, :pretty => true)
      else
